...
ls | tee doc-folder.txt
install -dvm 755 "$pkgdir/usr/share/doc/$pkgname"

# Create a samples subfolder
ls | tee samples-folder.txt
install -dvm 755 "$pkgdir/usr/share/doc/$pkgname/samples"

# Move samples to subfolder
ls | tee samples.txt
install -Dm 644 "$pkgdir/usr/samples/*" "$pkgdir/usr/share/doc/$pkgname/samples"
